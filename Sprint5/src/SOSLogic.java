...
public abstract class SOSLogic {
    ...
    public String getMoveRecordAsString() {
        StringBuilder result = new StringBuilder();
        result.append("[");
        for (int i = 0; i < moveRecord.size(); i++) {
            result.append(Arrays.toString(moveRecord.get(i)));
            if (i < moveRecord.size() - 1) {
                result.append(", "); // Add a comma between elements
            }
        }
        result.append("]");
    ...
    protected int calculateCompletedSequence() {
        ...
        for (int i = 0; i < boardSize; i++) {
            for (int j = 0; j < boardSize; j++) {
                ...
                if (j >= 2 && board[i][j] == 'S' && board[i][j - 1] == 'O'
                        && board[i][j - 2] == 'S') {
                    int[][] completedSeq = new int[][]{{i, j}, {i, j - 1}, {i, j - 2}};
                    if (!(containsSeq(currPlayer.getCompletedSequences(), completedSeq)
                            || containsSeq(
                            oppPlayer.getCompletedSequences(), completedSeq))) {
                        currPlayer.addCompletedSequence(completedSeq);
                        completedSeqDelta++;
                    }
                }
                // Check vertical seq
                if (i >= 2 && board[i][j] == 'S' && board[i - 1][j] == 'O'
                        && board[i - 2][j] == 'S') {
                ...
                if (i >= 2 && j >= 2 && board[i][j] == 'S' && board[i - 1][j - 1] == 'O'
                        && board[i - 2][j - 2] == 'S') {
                    int[][] completedSeq =
                            new int[][]{{i, j}, {i - 1, j - 1}, {i - 2, j - 2}};
                    if (!(containsSeq(currPlayer.getCompletedSequences(), completedSeq)
                            || containsSeq(
                            oppPlayer.getCompletedSequences(), completedSeq))) {
                        currPlayer.addCompletedSequence(completedSeq);
                ...
                if (i >= 2 && j <= boardSize - 3 && board[i][j] == 'S'
                        && board[i - 1][j + 1] == 'O' && board[i - 2][j + 2] == 'S') {
                    int[][] completedSeq =
                            new int[][]{{i, j}, {i - 1, j + 1}, {i - 2, j + 2}};
                    if (!(containsSeq(currPlayer.getCompletedSequences(), completedSeq)
                            || containsSeq(
    ...
    // Tries a move in a hypothetical scenario; does not impact the actual game
    // //True if complete seq move, False if no complete seq move
    public int tryMove(int row, int col, char letter) {
        ...
        tempBoard[row][col] = letter; // make hypothetical move on hypothetical
        // board

        for (int i = 0; i < boardSize; i++) {
            for (int j = 0; j < boardSize; j++) {
                // Check horizontal seq
                if (j >= 2 && tempBoard[i][j] == 'S' && tempBoard[i][j - 1] == 'O'
                        && tempBoard[i][j - 2] == 'S') {
                    int[][] completedSeq = new int[][]{{i, j}, {i, j - 1}, {i, j - 2}};
                    if (!(containsSeq(currPlayer.getCompletedSequences(), completedSeq)
                            || containsSeq(
                            oppPlayer.getCompletedSequences(), completedSeq)
                            || containsSeq(tempCompletedSeqs, completedSeq))) {
                        completedSeqDelta++;
                        tempCompletedSeqs.add(completedSeq);
                    }
                }
                // Check vertical seq on hypothetical board
                if (i >= 2 && tempBoard[i][j] == 'S' && tempBoard[i - 1][j] == 'O'
                        && tempBoard[i - 2][j] == 'S') {
                    int[][] completedSeq = new int[][]{{i, j}, {i - 1, j}, {i - 2, j}};
                    if (!(containsSeq(currPlayer.getCompletedSequences(), completedSeq)
                            || containsSeq(
                ...
                if (i >= 2 && j <= boardSize - 3 && tempBoard[i][j] == 'S'
                        && tempBoard[i - 1][j + 1] == 'O'
                        && tempBoard[i - 2][j + 2] == 'S') {
                    int[][] completedSeq =
                            new int[][]{{i, j}, {i - 1, j + 1}, {i - 2, j + 2}};
        ...
        return completedSeqDelta; // Returns the delta of completedSequences in
        // hypothetical board vs actual board
    }

    // Checks if a sequence exists in a list of sequences
    private static boolean containsSeq(
            ArrayList<int[][]> seqList, int[][] targetArray) {
        for (int[][] seq : seqList) {
